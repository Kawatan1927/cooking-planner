...
2. **適用可否の判定**
   ...
   - 次の場合は、そのコメントは **push対象外**（返信のみ）:
     ...
     - セキュリティ/コンプライアンス上の懸念
     - fork PR
     - protected files への変更が必要（safe-output の fallback-to-issue に委ねる）
...
7. **スレッド解決（条件付き）**
   - 実際にコードを修正して push した場合は、対応したスレッドを積極的に resolve する。
   - 修正をコミットしていないスレッド（返信のみ）はresolveしない。

## 出力ポリシー

- 対応可能な修正は積極的にpushする。push禁止条件に該当する場合のみ返信のみで終了する。
