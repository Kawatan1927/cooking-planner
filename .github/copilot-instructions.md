...
`gh issue develop` を使い、Issue の Development 欄に自動リンクされるブランチを作成します。

```bash
gh issue develop <Issue番号> --name <ブランチ名> --base <ベースブランチ> --checkout
```
...
```
例: 123-add-search-feature
```

### ベースブランチ

- 通常: `main`
- 大型リリース: `milestone/*`（例: `milestone/v4.0.0`）
...
```bash
gh pr create --title "PR タイトル" --body "closes #<Issue番号>" --label <ラベル>
```

### PR ルール

1. **PR タイトル** = 対応する Issue と同じタイトルにする
2. **PR 本文**に `closes #N` を記載して Issue を自動クローズする
3. `--label` フラグで適切なラベルを付与する
4. PR テンプレート（`.github/pull_request_template.md`）に従い、Issue 番号と変更内容の説明を記載する

## 1. リポジトリ全体の前提

-  生成する文章（PRタイトル/本文、レビューコメント、コミットメッセージ、要約、チェックリスト等）はすべて日本語で書いてください。英語での説明は書かないでください。- このプロジェクトは **個人用のWebアプリケーション** です。
