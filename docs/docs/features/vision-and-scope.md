...
「料理本ベースのレシピを、献立と買い物リストまで一気通貫で管理できる個人専用 Web アプリ」。
...
- 技術リテラシーは高い想定（自分用のため、UI/UX は必要十分レベルを目標）。
...
- 外部からアクセスできる URL でホストするが、
  **ログインした自分以外は使えない（Cognito + JWT ベースの認証）**
...
- 認証
  ...
  - ログイン済みユーザーのみ API にアクセスできる（API Gateway + JWT Authorizer）
- デプロイ
  - インターネットからアクセス可能な URL
  - HTTPS 対応（CloudFront）
...
- 可用性
  - 「たまに Lambda のコールドスタートで少し遅い」程度は許容
- セキュリティ
  - 外部から API を叩くには Cognito でログイン済みの JWT が必須
  - HTTPS 通信のみ（CloudFront + ACM）
- 運用
  - CloudWatch Logs で最低限のログを確認できること
