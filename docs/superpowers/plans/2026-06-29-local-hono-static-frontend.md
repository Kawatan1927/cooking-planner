...
- Modify: `backend/src/shoppingList/getShoppingList.test.ts`
  - 非 `/api` の業務 API 廃止に合わせ、app 経由のリクエストを `/api/*` に更新する。
- Modify: `backend/src/app.test.ts`
  - テスト用の一時ディレクトリに `index.html` と静的アセットを作成し、
    `FRONTEND_DIST_DIR` で配信元を差し替える。
...
beforeAll(async () => {
  ...
  frontendDistDir = await mkdtemp(
    join(tmpdir(), "cooking-planner-frontend-dist-"),
  );
...
- The test must not create, overwrite, or remove the real `frontend/dist` directory.
...
app.all("/api/*", () =>
  resultToResponse(notFound("Endpoint not found", "RESOURCE_NOT_FOUND")),
);

app.use("*", serveStatic({ root: FRONTEND_DIST_DIR }));
app.get(
  "*",
  serveStatic({ root: FRONTEND_DIST_DIR, path: FRONTEND_INDEX_HTML }),
);
...
- Include:
  ...
  - `backend/src/shoppingList/getShoppingList.test.ts`
...
```bash
git add backend/src/app.ts backend/src/app.test.ts backend/src/menus/updateMenu.test.ts backend/src/recipes/getRecipeById.test.ts backend/src/shoppingList/getShoppingList.test.ts frontend/.env.example frontend/.env.production.example docs/superpowers/plans/2026-06-29-local-hono-static-frontend.md
```
...
- Type consistency: `FRONTEND_DIST_DIR`、`FRONTEND_INDEX_HTML`、`protectedPaths` は Task 2 内で定義してから使用している。
